...
export default function ViewBookingsPage() {
  ...
  // 1. INITIAL FETCH (Equivalent to ngOnInit)
  useEffect(() => {
    const loadData = async () => {
      try {
        setLoading(true);
        // Replace with your actual API call: CIFwebService.getIserBookings(email)
        const response = await myAppWebService.GetUserAllBookingSlot(userData.EmailId); 
        const json = await response.json();
        setBookingData(json.item1 || []);
      } catch (error) {
  ...
  // 5. ACTION: PAYMENT CLICK (Redirect Logic)
  const handlePayment = (booking: any) => {
    // Logic from your .ts: Constructing the payment URL
    const paymentUrl = `/payment-gateway?bookingId=${booking.bookingId}&amount=${booking.totalCharges}`;
    window.location.href = paymentUrl; 
  };

  // 6. ACTION: UPLOAD RECEIPT (Modal Logic)
  const handleUploadClick = (booking: any) => {
    Swal.fire({
      title: 'Upload Payment Receipt',
      input: 'file',
      inputAttributes: { 'accept': 'image/*,application/pdf', 'aria-label': 'Upload your receipt' },
      showCancelButton: true,
      confirmButtonText: 'Upload',
      showLoaderOnConfirm: true,
      preConfirm: (file) => {
        // Here you would call your CIFwebService.UploadPaymentReceipt(formData)
        
        console.log("Uploading file for booking:", booking.bookingId, file);
      }
    }).then((result) => {
  ...
  return (
    <div className="p-8">
      <BookingTable 
        data={paginatedData}
        onSearch={(val: SetStateAction<string>) => { setSearchQuery(val); setCurrentPage(1); }}
        ...
        pagination={{
          ...
          onLimitChange: (limit: SetStateAction<number>) => { setItemsPerPage(limit); setCurrentPage(1); }
        }}
      />
    </div>
  );
